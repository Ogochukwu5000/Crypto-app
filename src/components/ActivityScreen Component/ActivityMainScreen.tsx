...
function ActivityMain(): JSX.Element {
    ...
    const renderItem = ({ item }: any) => (
        ...
        >
            ...
            <View>
                ...
                <Text style={styles.transactionCryptoAmount}>
                    {item.amount_crypto.toFixed(5)} ETH
                </Text>
            </View>
            <View>
    ...
    useEffect(() => {
        setLoading(true);
        axios
            .get(`${BASE_URL}transaction/${user?.cryptoTag}`)
            .then((response) => {
    ...
    const renderTransactions = () => {
        if (transactions.length === 0) {
            return <Text style={styles.noActivity}>No Activity At The Moment</Text>;
        } else {
            return (
                <FlatList
                    data={transactions}
